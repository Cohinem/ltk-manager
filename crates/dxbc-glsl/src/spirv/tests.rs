...
fn registers_take_their_rdef_names_and_the_interface_its_prefixes() {
    let patched = patch(&module(), &reflection(), Stage::Vertex).unwrap();
    assert_eq!(patched.renames["cb1"], "PerFrameVertexCB_i");
    assert_eq!(patched.renames["t3"], "Diffuse_Texture_TX");
    assert_eq!(patched.renames["s0"], "Diffuse_Texture_SMP");
    assert_eq!(patched.renames["POSITION"], "a_POSITION");
    assert_eq!(patched.renames["TEXCOORD"], "v_TEXCOORD");
    assert!(!patched.renames.contains_key("gl_Position"));
...
fn a_pixel_input_is_a_varying_and_its_output_keeps_its_name() {
    ...
    assert_eq!(patched.renames["POSITION"], "v_POSITION");
    assert!(!patched.renames.contains_key("TEXCOORD"));
}
