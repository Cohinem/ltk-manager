...
pub fn toc_path(object_path: &str, stage: Stage) -> String {
    ...
    format!(
        "assets/shaders/generated/{}.{stage}-dx11",
...
/// passes its whole define list. `defines` are `(name, value)` pairs, with an empty value
/// for a bare name.
#[must_use]
pub fn permutation(toc: &ShaderToc, defines: &[(String, String)]) -> Option<u32> {
    let mut kept: Vec<ShaderMacroDefinition> = defines
        .iter()
        .map(|(name, value)| ShaderMacroDefinition::new(name.clone(), value.clone()))
