//! Finding the `RiotClientServices.exe` that owns a given League install.
...
impl RiotClientInstalls {
    ...
    pub fn candidates(&self, league_root: Option<&Path>) -> Vec<String> {
        let mut out = Vec::new();

        if let Some(root) = league_root {
...
/// Normalise a League path into an `associated_client` lookup key.
///
...
/// the root.
fn installs_key(league_path: &Path) -> String {
    let key = normalize_key(&league_path.to_string_lossy());
...
/// Resolve the Riot Client exe for a League install.
///
