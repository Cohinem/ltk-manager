...
Issues and specs for this repo live as GitHub issues in `LeagueToolkit/ltk-manager`. Use the `gh`
...
"Commits and PRs" in the root `AGENTS.md` for the register a subject is written in.
...
Beyond the triage roles in `triage-labels.md`, this repo sorts issues on three axes. Reuse these
rather than inventing new labels.

- `area: backend`, `area: frontend`, `area: patcher`, `area: components`, `area: workshop`
- `type: performance`, `type: ci/cd`, `type: refactor`, `type: dx`, `type: ux`
