...
export function NativePage() {
  ...
  if (versionError || champQuery.error) {
    const msg =
      (versionError as Error | undefined)?.message ??
      (champQuery.error as Error | undefined)?.message ??
      "Unknown error";
    return (
      ...
      <div className="border-b border-surface-600 bg-surface-800/50 px-4 py-3">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="hidden items-center gap-1.5 text-xs text-surface-500 sm:inline-flex">
            <span className="rounded border border-surface-600 bg-surface-800 px-1 py-0.5 font-mono text-[11px]">
              Esc
            </span>{" "}
            to go back
          </span>
          ...
          <div className="grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-10">
            {champs.map((c) => (
              <button
                ...
                className="group text-left focus-visible:ring-2 focus-visible:ring-accent-500 focus-visible:ring-offset-2 focus-visible:ring-offset-surface-900 focus-visible:outline-none"
              >
                <div className="overflow-hidden rounded-lg border border-surface-700/70 bg-surface-800/60 transition-all group-hover:border-accent-500/40 group-hover:bg-surface-800">
                  ...
                  <div className="px-2 py-2">
                    <div className="truncate text-xs leading-tight font-medium text-surface-100 group-hover:text-white">
                      {c.name}
                    </div>
                    <div className="truncate text-[11px] leading-tight text-surface-500">
                      {c.title.replace(/^the\s+/i, "")}
                    </div>
                  </div>
                </div>
              </button>
...
function SkinSelector({
      ...
      <div className="flex h-full flex-col bg-[#070b10]">
        <div className="flex h-12 shrink-0 items-center px-4">
          <button
            type="button"
            onClick={onBack}
            className="inline-flex h-8 items-center gap-2 border border-[#c8aa6e]/45 bg-black/35 px-3 text-xs font-semibold tracking-[0.16em] text-[#c8aa6e] transition hover:bg-[#c8aa6e]/10"
          >
  ...
  const applySkin = async () => {
    ...
    try {
      ...
      toast.success(
        "Skin applied",
        `${activeChroma?.name ?? activeSkinName} is ready for the next patch.`,
      );
    } catch (error) {
      toast.error(
        "Couldn’t apply skin",
    ...
    <div className="relative flex h-full flex-col overflow-hidden bg-[#05080d] text-white">
      <div className="absolute inset-0">
        ...
        <div
          ...
          style={{
            background: `radial-gradient(900px 560px at 54% 28%, ${palette.glow}, transparent 72%)`,
          }}
        />
      </div>
      ...
      <div className="relative z-10 flex h-12 shrink-0 items-center px-4 sm:px-8">
        <button
          type="button"
          onClick={onBack}
          className="inline-flex h-8 items-center gap-2 border border-[#c8aa6e]/50 bg-black/35 px-3 text-xs font-semibold tracking-[0.2em] text-[#c8aa6e] uppercase shadow-[0_4px_18px_rgba(0,0,0,0.25)] backdrop-blur-sm transition hover:border-[#f0d9a0] hover:bg-[#c8aa6e] hover:text-[#101820] focus-visible:ring-2 focus-visible:ring-[#c8aa6e]/50 focus-visible:outline-none"
        >
          <ArrowLeft className="h-3.5 w-3.5" /> Champions
        </button>
      </div>

      <div className="relative z-10 flex min-h-0 flex-1 flex-col justify-between gap-8 px-4 pt-8 pb-6 sm:px-8 lg:pt-14">
        <div className="max-w-xl">
          <div className="flex flex-wrap items-center gap-2">
            {detail.tags.map((tag) => (
              <span
                key={tag}
                className="border border-white/15 bg-black/25 px-2 py-0.5 text-[10px] tracking-[0.2em] text-white/65 uppercase backdrop-blur-sm"
              >
                {tag}
              </span>
            ))}
          </div>
          <h1 className="mt-3 font-serif text-5xl leading-none font-bold tracking-[0.04em] text-[#f0d9a0] uppercase drop-shadow-[0_3px_15px_rgba(0,0,0,0.8)] sm:text-7xl">
            {detail.name}
          </h1>
          <p className="mt-1 text-sm text-white/65 capitalize italic sm:text-base">
            {detail.title}
          </p>
          <div className="mt-5 flex items-center gap-3">
            <span className="h-px w-10 bg-[#c8aa6e]/70" />
            <p className="text-lg font-semibold text-white drop-shadow sm:text-2xl">
              {activeChroma?.name ?? activeSkinName}
            </p>
          </div>
          {activeTier && (
            <p className="mt-1 text-[11px] tracking-[0.3em] text-[#c8aa6e] uppercase">
              {activeTier}
            </p>
          )}
          <button
            ...
            className="mt-5 inline-flex h-10 items-center gap-2 border border-[#c8aa6e]/70 bg-[#c8aa6e] px-4 text-xs font-bold tracking-[0.18em] text-[#101820] uppercase shadow-[0_8px_25px_rgba(0,0,0,0.3)] transition hover:bg-[#f0d9a0] disabled:cursor-not-allowed disabled:border-white/20 disabled:bg-black/35 disabled:text-white/45 disabled:shadow-none"
          >
            {applying ? (
              <Loader2 className="h-4 w-4 animate-spin" />
            ) : appliedSkinKey === appliedKey ? (
        ...
        {activeHasChromas && (
          ...
          >
            {chromasOpen && previewChroma && (
              <div className="pointer-events-none absolute bottom-full left-1/2 z-30 mb-4 w-[min(80vw,26rem)] -translate-x-1/2 overflow-hidden border border-[#c8aa6e]/60 bg-[#071019]/95 p-1.5 shadow-[0_12px_40px_rgba(0,0,0,0.7)] backdrop-blur-md">
                <img
                  src={previewSrc}
                  alt={previewChroma.name}
                  className="max-h-[min(60vh,24rem)] w-full object-contain"
                />
                <p className="truncate px-2 py-1.5 text-center text-sm font-semibold text-[#f0d9a0]">
                  {previewChroma.name}
                </p>
              </div>
            )}
            {chromasOpen && (
              <div
                role="group"
                aria-label={`Chromas for ${activeSkinName}`}
                className="mb-3 flex max-w-[min(90vw,34rem)] flex-wrap items-center justify-center gap-2.5"
                onMouseLeave={() => setPreviewChromaId(null)}
              >
                <button
                  type="button"
                  onClick={() => setChromaIdx(null)}
                  onMouseEnter={() => setPreviewChromaId(null)}
                  aria-pressed={chromaIdx === null}
                  title={`${activeSkinName} — base colors`}
                  className={`grid h-9 w-9 shrink-0 place-items-center rounded-full text-[9px] font-semibold tracking-wider uppercase transition-all ${chromaIdx === null ? "scale-110 bg-[#c8aa6e] text-[#101820] shadow-[0_0_18px_rgba(200,170,110,0.7)]" : "bg-[#172331] text-white/70 ring-1 ring-white/15 hover:ring-[#c8aa6e]"}`}
                >
                  Base
                </button>{" "}
                {chromas.map((chroma, index) => {
                  const first = chroma.colors[0] ?? "#777";
                  const second = chroma.colors[1];
                  const isSelected = chromaIdx === index;
                  return (
                    <button
                      key={chroma.id}
                      type="button"
                      onClick={() => setChromaIdx(index)}
                      onMouseEnter={() => setPreviewChromaId(chroma.id)}
                      onFocus={() => setPreviewChromaId(chroma.id)}
                      aria-pressed={isSelected}
                      aria-label={chroma.name}
                      title={chroma.name}
                      className={`h-9 w-9 shrink-0 rounded-full ring-1 transition-all duration-200 outline-none hover:scale-125 focus-visible:scale-125 focus-visible:ring-2 focus-visible:ring-[#c8aa6e] ${isSelected ? "scale-125 shadow-[0_0_18px_rgba(200,170,110,0.7)] ring-2 ring-[#c8aa6e]" : "ring-white/20 hover:ring-[#c8aa6e]"}`}
                      style={{
                        background:
            ...
            <button
              type="button"
              onClick={() => setChromasOpen((open) => !open)}
              aria-expanded={chromasOpen}
              aria-label={chromasOpen ? "Hide chromas" : `Show ${chromaCount} chromas`}
              className={`group relative grid h-14 w-14 place-items-center rounded-full transition-all duration-300 outline-none focus-visible:ring-2 focus-visible:ring-[#c8aa6e] ${chromasOpen ? "scale-105" : "hover:scale-110"}`}
            >
              <span
              ...
              <span className="absolute -right-0.5 -bottom-0.5 rounded-full bg-[#071019] px-1.5 text-[10px] font-semibold text-[#c8aa6e] ring-1 ring-[#c8aa6e]/50">
                {chromaCount}
              </span>
            </button>
          </div>
      ...
      <div className="relative z-10 flex shrink-0 items-stretch border-t border-[#c8aa6e]/25 bg-[#071019]/90 px-2 py-2 backdrop-blur-md sm:px-6 sm:py-3">
        <button
          ...
          className="hidden w-10 shrink-0 place-items-center border border-white/10 bg-black/25 text-[#c8aa6e] transition hover:bg-[#c8aa6e] hover:text-[#101820] sm:grid"
        >
          <ChevronLeft className="h-4 w-4" />
        </button>
        <div
          ref={carouselRef}
          className="flex min-w-0 flex-1 gap-3 overflow-x-auto px-2 pt-1 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
          role="listbox"
          aria-label={`${detail.name} skins`}
        >
          {skins.map((skin, index) => {
            ...
            return (
              <button
                key={skin.id}
                type="button"
                onClick={() => setSkinIdx(index)}
                aria-pressed={selectedSkin}
                title={label}
                className={`group gold-frame relative w-36 shrink-0 overflow-hidden border border-[#c8aa6e]/45 bg-[#0d1721] text-left transition-all duration-200 outline-none hover:-translate-y-1 focus-visible:-translate-y-1 focus-visible:ring-2 focus-visible:ring-[#c8aa6e] sm:w-44 ${selectedSkin ? "-translate-y-1 border-[#c8aa6e] shadow-[0_0_24px_rgba(200,170,110,0.45)] ring-1 ring-[#c8aa6e]" : ""}`}
              >
                <div className="relative aspect-[16/9] overflow-hidden">
                  <img
                    src={skin.tilePath || skin.splashPath}
                    alt={label}
                    loading="lazy"
                    className="h-full w-full object-cover object-top transition-transform duration-500 group-hover:scale-105"
                  />
                  <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-[#05080d] via-[#05080d]/20 to-transparent" />
                </div>
                <div className="space-y-1 p-2">
                  ...
                  <div className="flex flex-wrap items-center gap-1">
                    {skin.isBase && (
                      <span className="rounded-sm bg-[#172331] px-1.5 py-0.5 text-[9px] tracking-wider text-white/55 uppercase">
                        Base
                      </span>
                    )}
                    {tier && (
                      <span className="rounded-sm bg-[#c8aa6e]/20 px-1.5 py-0.5 text-[9px] tracking-wider text-[#c8aa6e] uppercase">
                        {tier}
                      </span>
                    )}
                    {count > 0 && (
                      <span className="inline-flex items-center gap-1 rounded-sm bg-[#4a89aa]/20 px-1.5 py-0.5 text-[9px] tracking-wider text-[#8ecbe8] uppercase">
