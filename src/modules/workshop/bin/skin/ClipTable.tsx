...
import { playableClips } from "./skinScene";
import type { GraphSource } from "./useGraphSource";
...
function ClipTable({ graph, source }: { graph: AnimationGraph; source: GraphSource }) {
  ...
  const rows = useMemo(() => matchingClips(graph.clips, filter), [graph.clips, filter]);
  ...
  useEffect(() => {
    ...
    const index = rows.findIndex((row) => row.hash === mark.hash);
